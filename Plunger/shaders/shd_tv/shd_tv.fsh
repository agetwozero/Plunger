...
void main()
{
    ...
    col.r = texture2D(gm_BaseTexture, vec2(uv.x, uv.y)).x;
    col.g = texture2D(gm_BaseTexture, vec2(uv.x, uv.y)).y;
    col.b = texture2D(gm_BaseTexture, vec2(uv.x, uv.y)).z;

    col = clamp(col * 0.5 + 0.6 * col * col, 0.0, 1.0);
    col *= 0.6 + 6.4 * uv.x * uv.y * (1.0 - uv.x) * (1.0 - uv.y); // Vignetting

    //col *= vec3(0.9, 1.0, 0.7); // "TV Color"
    col *= vec3(0.6, 1.5, 0.3); // "TV Color"
